...
export function ImageField({ field, onUpdate, onDelete }: ImageFieldProps) {
  return (
    <div className="space-y-3 p-3 border rounded-lg bg-stone-50/30">
      <div className="flex items-center justify-between">
        <Label className="text-sm font-medium">Campo de Imagem</Label>
        <Button
          variant="ghost"
          size="sm"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            placeholder="Ex: Avatar do Personagem"
          />
        </div>
        
        <div>
          <Label htmlFor={`default-${field.id}`} className="text-xs">URL Padrão</Label>
          <Input
            id={`default-${field.id}`}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label htmlFor={`max-size-${field.id}`} className="text-xs">Tamanho Máximo (MB)</Label>
          <Input
            id={`max-size-${field.id}`}
            type="number"
            value={field.options?.[0] || '5'}
            onChange={(e) => {
              const options = field.options || ['5', '']
