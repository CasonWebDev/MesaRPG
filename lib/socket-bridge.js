...
function initSocketServer(httpServer) {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('token_move', async (data) => {
      try {
        ...
        if (socket.data.campaignId !== campaignId) {
          ...
          console.error(`❌ Rejecting token_move event due to campaign mismatch`)
          socket.emit('error', 'Not in this campaign')
          return
        }
        ...
        let gameStateForUpdate = await prisma.gameState.findUnique({
          where: { campaignId }
        })

        if (!gameStateForUpdate) {
          console.log(`📝 Creating new gameState for campaign ${campaignId}`)
          gameStateForUpdate = await prisma.gameState.create({
            data: {
              campaignId,
              tokens: JSON.stringify([]),
              gameData: JSON.stringify({}),
        ...
        const tokens = JSON.parse(gameStateForUpdate.tokens || '[]')
    ...
    // Map freeze/unfreeze (GM only)
    socket.on('map:freeze', async (data) => {
      try {
        const { campaignId, frozen } = data
        console.log(`🔒 Map ${frozen ? 'freeze' : 'unfreeze'} request in campaign ${campaignId}`)
        ...
        if (!campaign || campaign.owner.id !== socket.data.userId) {
          socket.emit('error', 'Only GM can freeze/unfreeze maps')
          return
        }

        // Update GameState with freeze status
        const gameState = await prisma.gameState.upsert({
          where: { campaignId },
          update: {
            mapFrozen: frozen,
            frozenBy: frozen ? socket.data.userId : null,
        ...
        io.to(campaignId).emit('map:frozen-changed', {
          campaignId,
          mapFrozen: frozen,
          frozenBy: socket.data.userId,
          frozenByName: socket.data.userName,
          frozenAt: frozen ? new Date().toISOString() : null,
          timestamp: new Date().toISOString()
        })

        console.log(`✅ Map ${frozen ? 'frozen' : 'unfrozen'} by ${socket.data.userName} in campaign ${campaignId}`)

      } catch (error) {
        console.error('❌ Error freezing/unfreezing map:', error)
        socket.emit('error', 'Failed to freeze/unfreeze map')
      }
    })
    ...
    // Character update event handler (for token synchronization)
    socket.on('character:updated', async (data) => {
      try {
        const { campaignId, characterId, name, avatar } = data
        console.log(`👤 Character update received: ${characterId} in campaign ${campaignId}`)
        ...
        socket.to(campaignId).emit('character:updated', {
          characterId,
          campaignId,
          name,
          avatar,
        ...
        console.log(`✅ Character update broadcasted for ${characterId}`)
      } catch (error) {
